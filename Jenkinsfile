pipeline {
    ...
    stages {
        ...
        stage('Post build action') {
            steps {
                sh ' aws deploy update-deployment-group --application-name MQTTS-SREE --current-deployment-group-name produc-sree --ec2-tag-filters Key=TODAY,Value=NEWSERVER2019,Type=KEY_AND_VALUE --region us-east-1'
            } 
